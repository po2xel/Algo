...
void PrintMatrix(const Matrix<T, N, N>& aMatrix)
{
    for (auto& lRow : aMatrix)
    {
        std::copy(lRow.begin(), lRow.end(), std::ostream_iterator<T>(std::cout, "\t"));
        std::cout << '\n';
    }
}
...
template <typename T, std::size_t N>
Matrix<T, N> GenerateMagicSquare(void)
{
    std::cout << "The magic number is: " << N * (N * N + 1) / 2 << "\n";

    Matrix<T, N> lMatrix{};

    auto lRowIdx = static_cast<int>(N / 2), lColIdx = static_cast<int>(N - 1);

    for (T lIdx = 1; lIdx <= N * N; ++lIdx)
    {
        if (lRowIdx == -1 && lColIdx == N)
        {
            lRowIdx = 0;
            lColIdx = N - 2;
        }
...
{
    const auto& lMatrix = GenerateMagicSquare<int, 7>();
